...
// Інтерфейс для пропсів PlayStation
interface PlayStationProps {
  filteredByPlayStation: Game[];
  MdEuro: IconType;
...
const PlayStation: FC<PlayStationProps> = ({
  filteredByPlayStation,
  dispatch,
  increment,
}) => {
  ...
  const addedGames = useSelector(
    (state: { counter: { addedGames: Game[] } }) => state.counter.addedGames
  );
  ...
  return (
    <section className="wrapper-Xbox">
      <div className="wrapper_header">
        <span>
          <h1>PlayStation Ігри</h1>
        </span>
      </div>

      {filteredByPlayStation.map((game) => (
        <div className="Search_List_Item" key={game.id}>
          <Link
            to={`/About/${game.id}`}
            state={{ games: filteredByPlayStation }}
          >
