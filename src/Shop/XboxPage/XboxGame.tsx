...
interface HeloProps {
  ...
  MdEuro: any;
  increment: ActionCreatorWithPayload<SteamDeckData, "counter/increment">;
  filteredByGenre: SteamDeckData[];
}

const Xbox: FC<HeloProps> = ({ filteredByGenre, increment }) => {
      ...
      {filteredByGenre.map(
        (game: {
          ...
          <section className="wrapper-list">
            <div className="wrapper-Search_List_Item">
            <div className="Search_List_Item" key={game.id}>
              <Link to={`/About/${game.id}`} state={{ games: filteredByGenre }}>
