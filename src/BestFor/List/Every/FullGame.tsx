...
const FullGame = ({
  ...
  const addedGames = useSelector(
    (state: { counter: { addedGames: GameI[] } }) => state.counter.addedGames
  );

  return (
    <>
      {FulteredGames.length > 0 ? (
        <div className="List-section">
          {FulteredGames.map((game: GameI) => (
            <div
            ...
            >
              <Link to={`/About/${game.id}`} state={{ games: FulteredGames }}>
                {" "}
                {/*
            Передача даних */}
                <img
